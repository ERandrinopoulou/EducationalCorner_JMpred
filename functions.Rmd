...
calJM <- function(object, newdata, Tstart, Thoriz, idVar = "id", 
  ...
  matCens <- data.frame(years = newdata.id$years[newdata.id$id %in% names(Surv.censThoriz)], pred_prob = 1 - Surv.censThoriz, status = 1 - statusCens)
  
  # New part for calibration plot
  matDead <- data.frame(years = newdata.id$years[newdata.id$id %in% names(Surv.deadThoriz)], pred_prob = 1 - Surv.deadThoriz, status = 1)
  matAlive <- data.frame(years = newdata.id$years[newdata.id$id %in% names(Surv.aliveThoriz)], pred_prob = 1 - Surv.aliveThoriz, status = 0)
  ...
  #  The complementary log-log transformation for the predicted probabilities rather than the probabilities themselves.
  mat$cox.cll <- log(-log(1 - mat$pred_prob))
  # Use a conventional Cox proportional hazards model with restricted cubic splines to model the relationship between  
  ...
  predict.grid.cox <- seq(quantile(mat$pred_prob, probs = 0.01),
                          quantile(mat$pred_prob, probs = 0.99), length  = 100)
